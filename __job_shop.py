    ...
    def arrival(self, jtype):
        while True:
            if self.output <= 4:
                ...
                arrival_interval = int(np.random.exponential(arrival_interval))
...
class Sink:
    ...
    def job_arrive(self, job):
        ...
        if self.fac.throughput == 5:#job_num:
...
#factory
class Factory:
    def __init__(self, env, AV1, AV2, AV3, DD, job_info, DP_rule, log = True):
        self.env = env
        self.log = log
        #attribute
        self.AV1 = AV1
        ...
        self.routing_dict = {"a": self.job_info.loc[(self.job_info["jtype"] == "a")]["routing"][0],
                             "b": self.job_info.loc[(self.job_info["jtype"] == "b")]["routing"][1],
                             "c": self.job_info.loc[(self.job_info["jtype"] == "c")]["routing"][2],}
        self.PT_dict      = {"a": self.job_info.loc[(self.job_info["jtype"] == "a")]["process_time"][0],
                             "b": self.job_info.loc[(self.job_info["jtype"] == "b")]["process_time"][1],
                             "c": self.job_info.loc[(self.job_info["jtype"] == "c")]["process_time"][2],}
...
if __name__ == '__main__':
    ...
    #parameter
    simulationTime = 60 #1440 * 5
    job_info = pd.read_excel(input_dir + "job_shop_information.xlsx")
    ...
    fac = Factory(env, AV1, AV2, AV3, DD, job_info, DP_rule, log = True)
    fac.initialize()
    #run simulation
    ...
    print("Average cycle time: {}".format(np.mean(fac.sink.job_statistic['flow_time'])))
